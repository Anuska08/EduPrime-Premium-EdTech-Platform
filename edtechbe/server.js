...
const runSeed = async () => {
  ...
  await User.create({ name: 'Admin', email: 'admin@eduprime.com', password: 'Admin1234', role: 'admin' });
  ...
  await LiveClass.insertMany([
    { title: 'Live: Gravitation – Final Revision', subject: 'Physics', instructorName: teacher1.name, instructor: teacher1._id, scheduledAt: new Date(new Date(tomorrow).setHours(15, 0, 0)), color: '#6060f0', duration: 90 },
    { title: 'Live: Constitutional Amendments', subject: 'Polity', instructorName: teacher4.name, instructor: teacher4._id, scheduledAt: new Date(new Date(tomorrow).setHours(18, 30, 0)), color: '#f59e0b', duration: 60 },
    { title: 'Live: Pandas & Data Wrangling', subject: 'Python', instructorName: teacher3.name, instructor: teacher3._id, scheduledAt: new Date(new Date(dayAfter).setHours(17, 0, 0)), color: '#10b981', duration: 75 },
  ]);
...
const startServer = async () => {
  // Run seed if --seed flag or SEED=true env is set
  if (process.argv.includes('--seed') || process.env.SEED === 'true') {
  ...
  app.listen(PORT, () => {
    ...
    console.log('   To seed data, restart with: node server.js --seed');
  });
};
